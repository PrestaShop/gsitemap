...
class Gsitemap extends Module
{
    ...
    protected function _getProductLink(&$link_sitemap, $lang, &$index, &$i, $id_product = 0)
    {
        ...
        foreach ($products_id as $product_id) {
            ...
            $url = $link->getProductLink(
                ...
                htmlspecialchars(strip_tags($product->category)),
                $product->ean13,
                (int)$lang['id_lang'],
            ...
            if (isset($id_image['id_image'])) {
                $image_link = $this->context->link->getImageLink(
                    $product->link_rewrite,
                    $product->id.'-'.(int)$id_image['id_image'],
    ...
    protected function _getCategoryLink(&$link_sitemap, $lang, &$index, &$i, $id_category = 0)
    {
        ...
        foreach ($categories_id as $category_id) {
            ...
            if ($category->id_image) {
                $image_link = $this->context->link->getCatImageLink(
                    $category->link_rewrite,
                    (int)$category->id_image,
    ...
    protected function _recursiveSitemapCreator($link_sitemap, $lang, &$index)
    {
        ...
        foreach ($link_sitemap as $key => $file) {
            ...
            $this->_addSitemapNode(
                $write_fd,
                htmlspecialchars(strip_tags($file['link'])),
                $this->_getPriorityPage($file['page']),
                Configuration::get('GSITEMAP_FREQUENCY'),
                $lastmod
            );
            if ($file['image']) {
                $this->_addSitemapNodeImage(
                    $write_fd,
                    htmlspecialchars(strip_tags($file['image']['link'])),
                    isset($file['image']['title_img']) ? htmlspecialchars(
                        str_replace(
                            array(
                                "\r\n",
                                "\r",
    ...
    protected function _createIndexSitemap()
    {
        ...
        file_put_contents(
            $this->normalizeDirectory(_PS_ROOT_DIR_).$this->context->shop->id.'_index_sitemap.xml',
            $xml_feed->asXML()
        );
